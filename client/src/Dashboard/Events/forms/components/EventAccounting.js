...
const EventAccounting = (props) => {
    ...
    const submitHandler = (e) => {
        axios
            .post(`process.env.REACT_APP_URL:8080/api/bookings`, ({ data: input, event_id: event_id, producer_id: producer_id, artist_status: "pending", producer_status: "draft" }))
            .then((res) => {
                console.log(res.data);
                updateBookings()
            })
    ...
    useEffect(() => {
        axios
            .get(`process.env.REACT_APP_URL:8080/api/bookings/events/${event_id}`)
            .then((res) => {
    ...
    const removeBooking = (id) => {
        axios.delete(`process.env.REACT_APP_URL:8080/api/bookings/${id}/`)
        .then((res)=>{
            console.log(res);
            updateBookings()
        })
